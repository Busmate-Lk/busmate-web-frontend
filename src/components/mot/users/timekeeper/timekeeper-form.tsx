...
export default function TimekeeperForm({ timekeeperId, onSuccess, onCancel }: TimekeeperFormProps) {
    ...
    useEffect(() => {
        ...
        const fetchAllStops = async (): Promise<any[]> => {
            // 1) Try a list endpoint that returns all stops
            const svc: any = BusStopManagementService as any;
            try {
                const listResp = await (svc.getAllStopsAsList?.() ?? svc.getAllStopsList?.() ?? svc.getAllStops?.());
                if (Array.isArray(listResp)) return listResp;
                if (Array.isArray(listResp?.content)) return listResp.content;
                if (Array.isArray(listResp?.data)) return listResp.data;
            } catch (err) {
                // ignore and try paginated approach
            }

            // 2) Fallback: fetch pages until exhausted
            const all: any[] = [];
            const pageSize = 200;
            let page = 0;
            while (true) {
                try {
                    // try common paginated method signatures
                    const resp = await (svc.getAllStops?.({ page, size: pageSize }) ??
                        svc.getStops?.({ page, size: pageSize }) ??
                        svc.list?.({ page, size: pageSize }));
                    const items = Array.isArray(resp) ? resp : (resp?.content ?? resp?.data ?? []);
        ...
        const load = async () => {
            try {
                ...
                const response: any[] = await fetchAllStops();
                ...
                const formattedStops = stopsArray.map(stop => ({
                    id: stop.id ?? stop.busStopId ?? stop.stop_id ?? stop._id ?? String(stop),
                    stopName: stop.stopName ?? stop.name ?? stop.displayName ?? stop.stop_name ?? 'Unnamed Stop',
                }));
                ...
                if (mounted) setLoadingBusStops(false);
            }
        };
